...
var AlugueisComponent = class _AlugueisComponent {
  ...
  extractErrorMessage(err) {
    if (err?.status === 403)
      return "Sem permiss\xE3o. Apenas gestores podem gerenciar alugu\xE9is.";
    if (err?.error?.error)
  ...
  deleteAluguel(id) {
    if (confirm("Tem certeza que deseja excluir este aluguel?")) {
      this.apiService.deleteAluguel(String(id)).subscribe({
        ...
        error: (err) => this.toast.error(err?.status === 403 ? "Sem permiss\xE3o" : "Erro ao excluir aluguel", err?.status === 403 ? "Apenas gestores podem excluir alugu\xE9is." : void 0)
      });
    }
  }
  static {
...
//# sourceMappingURL=chunk-WQ5QXU4F.js.map
